...
def build_icesheets_ui():
    ...
    def on_run(_=None):
        ...
        with log_out:
            ...
            print(f"[icesheets] mode    : {mode_dd.label}")
            print(f"[icesheets] backend : {backend_dd.label}")
            print(f"[icesheets] model   : {model_dd.label}")
    ...
    .icesee-title { font-size: 18px; font-weight: 700; margin: 6px 0 4px; }
    .icesee-subtitle { color: rgba(0,0,0,.68); margin-bottom: 16px; line-height: 1.5; }
    .icesee-h { font-size: 18px; font-weight: 800; margin: 2px 0 12px; }
    .icesee-card {
    ...
    .icesee-lbl { font-weight: 650; color: rgba(0,0,0,.82); padding-top: 7px; }
    .icesee-subtle { color: rgba(0,0,0,.60); font-size: 12px; margin-bottom: 6px; }
    .icesee-status {
    ...
    <div class="icesee-page">
      <div class="icesee-title">Run supported ice-sheet models without the ICESEE DA layer.</div>
      <div class="icesee-subtitle">
        Choose a backend, select a supported model, and launch model-only workflows
    ...
    page = W.VBox([W.HTML(css), header, row, actions_card], layout=W.Layout(width="100%"))
